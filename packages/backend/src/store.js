...
// Fires quota.threshold-reached once per threshold per guardian (not once
// per transaction above that threshold) — tracked in-memory alongside the
// guardian record so a contract sitting at 85% doesn't re-alert on every
// subsequent tx until it crosses the next threshold.
const QUOTA_THRESHOLDS = [0.8, 0.95, 1.0];
function maybeFireQuotaAlert(g) {
  if (g.addedBy === "admin" || g.txLimit === Infinity) return;
  const pct = g.txCount / g.txLimit;
  g._alertedThresholds = g._alertedThresholds || new Set();
  for (const threshold of QUOTA_THRESHOLDS) {
    if (pct >= threshold && !g._alertedThresholds.has(threshold)) {
      g._alertedThresholds.add(threshold);
