...
// Quota threshold alerting: tracked in-memory (not a DB column) since it's
// just "have we already alerted for this contract at this % this process
// lifetime" — losing this on restart just means a possible one-time
// re-alert, not a correctness issue.
const QUOTA_THRESHOLDS = [0.8, 0.95, 1.0];
const alertedThresholds = new Map(); // key: contract address -> Set of thresholds fired
function maybeFireQuotaAlert(g, newCount) {
  if (g.addedBy === "admin" || g.txLimit === null) return;
  const pct = newCount / g.txLimit;
  if (!alertedThresholds.has(g.id)) alertedThresholds.set(g.id, new Set());
  const fired = alertedThresholds.get(g.id);
  for (const threshold of QUOTA_THRESHOLDS) {
    if (pct >= threshold && !fired.has(threshold)) {
      fired.add(threshold);
      dispatchAlert("quota.threshold-reached", {
...
export function recordEvent({ token, tokenAddress, from, to, severity, reason, rules, txHash, signed, hash, verdict, anchored }) {
  ...
  if (g && g.isActive) {
    ...
    if (signed && anchored) {
      ...
      maybeFireQuotaAlert(g, newCount);
    }
