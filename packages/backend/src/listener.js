...
import { notifyTelegramWatchers } from "./telegramDispatch.js";
...
async function processFlaggedResult(target, result, { txHash, from, to }) {
  ...
  if (result.risk !== "LOW" && gate.canMonitor) {
    ...
    // This is post-hoc detection — the transaction is already mined by the
    // time the listener sees it. Deliberately named "flagged," not
    // "blocked": nothing here prevented the transaction from happening.
    // "transaction.blocked" is reserved for the pre-signing path in
    // server.js, where a REVOKE_IMMEDIATELY recommendation is what makes
    // @shieldguard/sdk's ShieldGuardSigner actually throw before signing.
    dispatchAlert("transaction.flagged", {
      token: target.name,
      tokenAddress: target.address,
      txHash,
    ...
    // Ping anyone who's personally connected this contract in their
    // dashboard, on top of the generic webhook above.
    notifyTelegramWatchers(target.address, {
      severity: result.risk,
      reason: result.reason,
      txHash,
