...
// Every branch tags the verdict with which provider actually produced it
// (_provider). This is what lets callers fire an ai.fallback-triggered
// alert when the system falls all the way through to the deterministic
// template — ops needs to know when decisions stop being AI-reasoned and
// start being the conservative static ruleset, since that's a materially
// different confidence level even though the response shape is identical.
export async function generateVerdict(record) {
  if (process.env.GEMINI_API_KEY) {
    try {
      const v = await geminiVerdict(record);
      return { ...v, _provider: "gemini" };
    } catch (err) {
  ...
  if (process.env.OPENAI_API_KEY) {
    try {
      const v = await openAIVerdict(record);
      return { ...v, _provider: "openai" };
    } catch (err) {
  ...
  if (process.env.ANTHROPIC_API_KEY) {
    try {
      const v = await anthropicVerdict(record);
      return { ...v, _provider: "anthropic" };
    } catch (err) {
      console.error("[policyEngine] Anthropic failed, falling back:", err.message);
    }
  }
  return { ...templateVerdict(record), _provider: "template" };
}
